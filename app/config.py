...
class Settings(BaseSettings):
    ...
    # Global system prompt to shape assistant persona and safety behavior
    SYSTEM_PROMPT: str = Field(
        default=(
            "You are HealthLang — a friendly, helpful medical assistant.\n"
            "Style & Tone: Warm, conversational, clear. Use emojis sparingly "
            "when appropriate.\n"
            "Language: Respond in English by default. If the user explicitly "
            "asks for another language or translation, you may translate.\n"
            "Capabilities: You can answer general questions like a normal "
            "chatbot, and also provide medically informed guidance when the "
            "user asks health-related questions.\n"
            "Medical Safety:\n"
            "- Provide educational info, not diagnoses or prescriptions.\n"
            "- Encourage consulting a licensed clinician for personalized "
            "advice.\n"
            "- Escalate urgent symptoms with calm guidance to seek emergency "
            "care.\n"
            "- Be culturally sensitive and supportive.\n"
            "Conversation: Be empathetic and sentiment-aware; acknowledge "
            "feelings before answering.\n"
            "Formatting (when medical): Use short headings, bullets, and "
            "clear steps. Keep replies concise.\n"
            "If the question is non-medical, answer helpfully like a general "
            "assistant and you may add a brief note that HealthLang focuses "
            "on health."
        ),
        env="SYSTEM_PROMPT",
    )
    ...
    ADDITIONAL_EMBEDDING_MODELS: List[str] = Field(
        default=[],
        env="ADDITIONAL_EMBEDDING_MODELS",
    )
    ...
    @field_validator("LLM_PROVIDER", "MEDICAL_MODEL_PROVIDER", mode="before")
    @classmethod
    ...
    KNOWLEDGE_BASE_PATH: str = Field(
        default="./data/medical_knowledge/processed",
    ...
    CORS_ORIGINS: str = Field(
        default=(
            ...
            "https://lang-heal-chat.vercel.app"
        ),
        env="CORS_ORIGINS",
    )
    ...
    MCP_BASE_URL: str = Field(
        default="https://healthcare-mcp.onrender.com", env="MCP_BASE_URL"
    )
    MCP_API_KEY: str = Field(default="", env="MCP_API_KEY")
    MCP_HEALTHCARE_SERVER_PATH: str = Field(
        default="./mcp_servers/healthcare_server.py",
        env="MCP_HEALTHCARE_SERVER_PATH",
    )
    ...
    TEST_DATABASE_URL: str = Field(
        default="sqlite:///./test_healthlang.db",
    ...
    K8S_RESOURCE_LIMITS_CPU: str = Field(
        default="1000m",
        env="K8S_RESOURCE_LIMITS_CPU",
    )
    K8S_RESOURCE_LIMITS_MEMORY: str = Field(
        default="2Gi",
        env="K8S_RESOURCE_LIMITS_MEMORY",
    )
    K8S_RESOURCE_REQUESTS_CPU: str = Field(
        default="500m",
        env="K8S_RESOURCE_REQUESTS_CPU",
    )
    K8S_RESOURCE_REQUESTS_MEMORY: str = Field(
        default="1Gi",
        env="K8S_RESOURCE_REQUESTS_MEMORY",
    )
    ...
    def validate_settings(self) -> None:
        ...
        if self.is_production():
            if (
                ...
                or self.GROQ_API_KEY == "your_real_groq_api_key_here"
            ):
